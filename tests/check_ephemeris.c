...
START_TEST(test_ephemeris_kepler_equal)
{
  ...
  fail_unless(ephemeris_kepler_equal(&a, &b), "Ephemerides should be equal");

  a.tgd = 1;
  fail_unless(!ephemeris_kepler_equal(&a, &b),
...
Suite* ephemeris_suite(void)
{
  ...
  tcase_add_test(tc_core, test_ephemeris_kepler_equal);
